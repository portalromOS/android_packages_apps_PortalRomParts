...
public class ButtonBacklightBrightness extends CustomDialogPreference<AlertDialog> implements
    ...
    public ButtonBacklightBrightness(Context context, AttributeSet attrs) {
        ...
        if (DeviceUtils.hasKeyboardBacklightSupport(context)) {
            final boolean isSingleValue = KEYBOARD_BRIGHTNESS_TOGGLE_MODE_ONLY ==
                    context.getResources().getInteger(org.portalrom.platform.internal.R.integer
                            .config_deviceSupportsKeyboardBrightnessControl);
            mKeyboardBrightness = new BrightnessControl(
                    PortalRomSettings.Secure.KEYBOARD_BRIGHTNESS, isSingleValue);
    ...
    private int getTimeout() {
        return PortalRomSettings.Secure.getInt(mResolver,
                PortalRomSettings.Secure.BUTTON_BACKLIGHT_TIMEOUT, DEFAULT_BUTTON_TIMEOUT * 1000)
                / 1000;
    }

    private void applyTimeout(int timeout) {
        PortalRomSettings.Secure.putInt(mResolver,
                PortalRomSettings.Secure.BUTTON_BACKLIGHT_TIMEOUT, timeout * 1000);
    }
    ...
    private class BrightnessControl implements
        ...
        public float getBrightness(boolean persisted) {
            ...
            return PortalRomSettings.Secure.getFloat(mResolver, mSetting, mDefaultBrightness);
        }

        public void applyBrightness() {
            PortalRomSettings.Secure.putFloat(mResolver, mSetting, getBrightness(false));
        }
    ...
    private class ButtonBrightnessControl extends BrightnessControl {
        ...
        public boolean isOnlyWhenPressedEnabled() {
            return PortalRomSettings.System.getInt(mResolver, mOnlyWhenPressedSetting, 0) == 1;
        }

        public void setOnlyWhenPressedEnabled(boolean enabled) {
            PortalRomSettings.System.putInt(mResolver, mOnlyWhenPressedSetting, enabled ? 1 : 0);
        }
    }
}
