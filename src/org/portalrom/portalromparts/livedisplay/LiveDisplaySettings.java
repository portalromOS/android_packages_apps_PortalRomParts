 ...
 */
package org.portalrom.portalromparts.livedisplay;
...
import org.portalrom.portalromparts.search.SearchIndexableRaw;
import org.portalrom.portalromparts.search.Searchable;
import org.portalrom.portalromparts.utils.ResourceUtils;
...
import portalrom.preference.SettingsHelper;
import portalrom.providers.PortalRomSettings;

import static portalrom.hardware.LiveDisplayManager.FEATURE_ANTI_FLICKER;
import static portalrom.hardware.LiveDisplayManager.FEATURE_CABC;
import static portalrom.hardware.LiveDisplayManager.FEATURE_COLOR_ADJUSTMENT;
import static portalrom.hardware.LiveDisplayManager.FEATURE_COLOR_ENHANCEMENT;
import static portalrom.hardware.LiveDisplayManager.FEATURE_DISPLAY_MODES;
import static portalrom.hardware.LiveDisplayManager.FEATURE_PICTURE_ADJUSTMENT;
import static portalrom.hardware.LiveDisplayManager.FEATURE_READING_ENHANCEMENT;
...
public class LiveDisplaySettings extends SettingsPreferenceFragment implements Searchable,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String[] modeEntries = res.getStringArray(
                org.portalrom.platform.internal.R.array.live_display_entries);
        mModeValues = res.getStringArray(
                org.portalrom.platform.internal.R.array.live_display_values);
        mModeSummaries = res.getStringArray(
                org.portalrom.platform.internal.R.array.live_display_summaries);
        ...
        if (liveDisplayPrefs != null && mReadingMode != null &&
                !mHardware.isSupported(PortalRomHardwareManager.FEATURE_READING_ENHANCEMENT)) {
        ...
        if (liveDisplayPrefs != null && antiFlicker != null &&
                !mHardware.isSupported(PortalRomHardwareManager.FEATURE_ANTI_FLICKER)) {
    ...
    private void updateReadingModeStatus() {
        if (mReadingMode != null) {
            mReadingMode.setChecked(
                    mHardware.get(PortalRomHardwareManager.FEATURE_READING_ENHANCEMENT));
        }
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mLiveDisplay) {
            ...
            mHardware.set(PortalRomHardwareManager.FEATURE_READING_ENHANCEMENT, (Boolean) objValue);
        }
        return true;
    }
    ...
    public static final Searchable.SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
        ...
        public Set<String> getNonIndexableKeys(Context context) {
            ...
            if (!context.getResources().getBoolean(
                    org.portalrom.platform.internal.R.bool.config_enableLiveDisplay)) {
                result.add(KEY_LIVE_DISPLAY_TEMPERATURE);
                result.add(KEY_LIVE_DISPLAY);
            }
        ...
        public List<SearchIndexableRaw> getRawDataToIndex(Context context) {
            ...
            if (config.hasFeature(FEATURE_DISPLAY_MODES)) {
                DisplayMode[] modes = PortalRomHardwareManager.getInstance(context).getDisplayModes();
                if (modes != null && modes.length > 0) {
